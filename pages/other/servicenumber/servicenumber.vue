...
<script>
	...
	import provincesAndCitiess from '../../../components/provincesAndCities.js'
	...
	export default {
		...
		onReachBottom() {
			...
			const messages = UserInfo.subscribe.subscribeDesc
			console.log("messageIN === " + messages)
			console.log("this.message === " + this.message)
			if (messages == this.message) {
				this.loadget()
			}
		},
		onShow() {
				...
				if (!UserInfo.subscribe) {
						...
						success(res) {
							...
							if (res.data.code === 0) {
								...
								if (res.data.data.subscribe) {
									...
									uni.request({
										url: "https://zbger.com:8443/zbger/inviteTenders/findThirtyData",
										method: "GET",
										data: {
											provinceCode: provinceCode.join(),
											cityCode: cityCode.join(),
											announcementType: subscribeType,
											keyword: subscribeDesc
										},
										success(res) {
					...
					uni.request({
						...
						data: {
							provinceCode: provinceCode.join(),
							cityCode: cityCode.join(),
							announcementType: "不限" === subscribeType ? '' : subscribeType,
							keyword: subscribeDesc
						},
						success(res) {
							res.data.data.forEach(e => {
		...
		methods: {
			...
			async sear() {
				...
				const res = await this.$myRequest({
					url: '/zbger/inviteTenders/findThirtyData?provinceCode=' + this.provinceValue + '&cityCode=' + this.cityValue +
						'&announcementType=' + this.zType + '&day=' + this.zTime + '&keyword=' + this.message + '&pageNo=' + this.num
				});
			...
			/* det(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s){
				uni.navigateTo({
					url:"../notice/details?a="+a+"&b="+b+"&c="+c+"&d="+d+"&e="+e+"&f="+f+"&g="+g+"&h="+h+"&i="+i+"&j="+j+"&k="+k+"&l="+l+"&m="+m+
			...
			async loadget() {
				...
				const UserInfo = uni.getStorageSync("zbgerUserInfo")
				const cityValue = UserInfo.subscribe.areaCodes
				if (UserInfo.subscribe.subscribeType === "不限") {
					UserInfo.subscribe.subscribeType = ''
				}
				const zType = UserInfo.subscribe.subscribeType
				const messages = UserInfo.subscribe.subscribeDesc
				...
				const res = await this.$myRequest({
					url: '/zbger/inviteTenders/findTenData?provinceCode=' + cityValue + '&announcementType=' + zType + '&day=' +
						this.zTime + '&keyword=' + messages + '&pageNo=' + this.num,
				});
			...
			async get() {
				...
				const res = await this.$myRequest({
					url: '/zbger/inviteTenders/findTenData?provinceCode=' + this.cityValue + '&announcementType=' + this.zType +
						'&day=' + this.zTime + '&keyword=' + this.message + '&pageNo=' + this.num
